...
const stepDescriptions = {
  0: [ // 안드로이드 버전 확인
    "1. 화면을 아래에서 위로 쓸어 올려 앱 목록 화면으로 이동합니다.",
    "2. 설정 앱을 실행합니다.",
    "3. 설정 화면에서 태블릿 정보를 선택합니다.",
    "4. 소프트웨어 정보를 선택합니다.",
    "5. 안드로이드 버전을 확인합니다. 안드로이드 버전이 10 버전이 아닐 경우 고객센터로 연락 부탁드립니다."
  ],
  1: [ // 런처 설치 (20개)
    "1. 화면을 아래에서 위로 쓸어 올려 앱 목록 화면으로 이동합니다.",
    "2. 오른쪽 위의 돋보기를 터치합니다.",
    "3. Chrome 앱을 검색하여 실행합니다.",
    "4. 주소창에 <m.milkt.co.kr/sp/main>을 입력하여 이동한 후, '밀크T 런처'를 터치하여 다운로드합니다.",
    "5. 메인 화면으로 이동 후 내 파일 앱을 실행합니다.",
...
const additionalNotes = {
  1: { // 런처 설치
    5: "'취소'와 '설정'이 보이지 않고 '확인'만 노출될 경우, 자녀 보호 기능(패밀리 링크)앱에 의해 설치가 차단된 상황입니다. 태블릿에 설치된 자녀 보호 기능 앱 실행 > 우측 상단의 메뉴 터치 > 감독 중지를 통해 밀크T 태블릿을 관리 목록에서 제외시켜주세요",
    14: "'다음'이 보이지 않을 경우 와이파이 연결이 되지 않았거나 배터리가 30% 미만인 상황입니다. 충전기를 연결하면 배터리 30%가 되지 않더라도 '다음'버튼이 활성화됩니다.",
    16: "인증이 너무 오래 걸리거나 실패했다고 나온다면 와이파이 공유기 재부팅을 부탁드립니다. 그럼에도 해결되지 않을 경우, 번거로우시겠지만 밀크T 고객센터로 연락 부탁드립니다."
  }
};
...
window.addEventListener('load', function() {
  ...
  for (let tabIndex = 0; tabIndex < 2; tabIndex++) {
    ...
    if (slideImg) {
      slideImg.src = getImageSrc('s6lite', tabIndex, currentSlides[tabIndex]);
    }
...
function changeSlide(tabIndex, direction) {
  ...
  // 슬라이드 이미지 업데이트
  document.getElementById(`slideImage${tabIndex}`).src = getImageSrc('s6lite', tabIndex, currentSlides[tabIndex]);
...
function getImageSrc(device, tabIndex, slideIndex) {
  if (device === 's6lite' && tabIndex === 0) {
    // S6 Lite 안드로이드 버전 확인 (5장) - s6os 레포 사용
    if (slideIndex === 4) {
      // 4번 이미지만 ad 레포의 새로운 이미지로 교체
    ...
    // S6 Lite 런처 설치 (20장) - s6ins 레포 사용
    const paddedIndex = slideIndex.toString().padStart(2, '0');
    return `https://raw.githubusercontent.com/skyhigh79/s6ins/main/s6_launcher_install_${paddedIndex}.png`;
  }
